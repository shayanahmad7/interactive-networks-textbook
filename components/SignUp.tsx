...
export default function SignUp({ setShowSignUp }: SignUpProps) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    // Clean implementation: Explicitly redirect to dashboard after OAuth
    console.log('🎯 Using clean Google OAuth implementation with explicit redirect')
    
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: 'https://interactive-networks-textbook.vercel.app/dashboard'
      }
    })
    if (error) {
      alert(error.message)
